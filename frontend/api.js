...
const BASE_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
  ...
  : 'https://internhub-api.onrender.com/api';         // Production (Render)
...
async function apiFetch(endpoint, options = {}, _retry = false) {
  ...
  if (res.status === 401 && data.code === 'TOKEN_EXPIRED' && !_retry) {
    ...
    if (!isRefreshing) {
      ...
      if (refreshed) {
        ...
        // CRITICAL: resolve all queued promises with error — never leave them hanging
  ...
  // ── Other 401 (invalid/missing token) ──────────────────
  if (res.status === 401) {
    // Only clear session if we actually had a token (i.e. it's truly invalid)
    // Don't redirect here — let the calling page handle it.
    // Public pages (internships.html) should not be kicked to login just
    // because an optional auth call (getSaved) returns 401.
    Auth.clearSession();
...
const AuthAPI = {
  ...
  async toggleSave(internshipId) {
    return apiFetch(`/auth/save/${internshipId}`, { method: 'POST' });
  },
  async getSaved() {
    return apiFetch('/auth/saved');
  },
};
...
window.addEventListener('error', function(event) {
  ...
  errDiv.style.cssText = 'position:fixed;top:0;left:0;z-index:999999;background:red;color:white;padding:10px;width:100%;font-size:12px;';
  errDiv.innerHTML = `<b>JS ERROR:</b> ${event.message} <br> ${event.filename}:${event.lineno}:${event.colno}`;
  document.body.appendChild(errDiv);
});
window.addEventListener('unhandledrejection', function(event) {
  const errDiv = document.createElement('div');
  errDiv.style.cssText = 'position:fixed;top:50px;left:0;z-index:999999;background:orange;color:black;padding:10px;width:100%;font-size:12px;';
  errDiv.innerHTML = `<b>PROMISE REJECTION:</b> ${event.reason}`;
  document.body.appendChild(errDiv);
});
...
async function initAuthPage(requiredRole) {
  ...
  if (!result.success) {
    // Session is invalid/expired and refresh failed
    window.location.href = 'login.html';
    return null;
  }
