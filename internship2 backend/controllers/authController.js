...
// @desc    Toggle saving an internship
// @access  Private
// ─────────────────────────────────────────
exports.toggleSave = async (req, res, next) => {
  try {
    ...
    const isSaved = user.savedInternships.includes(internshipId);
    ...
    res.status(200).json({
      success: true,
      isSaved: !isSaved,
      message: !isSaved ? 'Internship saved!' : 'Removed from saved internships.',
...
exports.getSaved = async (req, res, next) => {
  try {
    ...
    // Filter out nulls in case an internship was deleted
    const validInternships = user.savedInternships.filter(i => i != null);
    
    res.status(200).json({
      success: true,
      count: validInternships.length,
      data: validInternships
    });
  } catch (error) {
    next(error);
  }
